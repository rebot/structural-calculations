...
# ╔═╡ bc37f850-37fc-424c-a051-5d3af96f5910
begin
    import Pkg
...
# ╔═╡ 5239a5bf-45c8-4c0d-9f5a-70e8184f30b6
md"""
Resulterende krachten ter hoogte van de steunpunten voor afdracht naar **kolom 1** en **kolom 2**
"""
...
@drawsvg begin
	...
	@layer (
		...
		Luxor.label("R₃", :W, pnt_x, offset=5);
	)
	@layer (
		fontsize(16);
...
p1, p2, p3, R3, x_steun = symbols("p_1 p_2 p_3 R_3 x_{steun}", real=true)
...
Bereking van **$naam**, een eenvoudig opgelegde ligger **onder muur 10**. Het profiel hoort in theorie de vloer van de badkamer niet te ondersteunen, gezien deze afdraagt van muur 1 naar muur 4 en 5. Veiligheidshalve rekenen we 20% mee van de last t.g.v. de vloer van de badkamer. Verder wordt wel de vloer van de hal gedragen door de muur. De krachtsafdracht bedraagt zo'n 40% van last t.g.v. de vloer van de hal. 
...
M = M1(deel1...) + M1(deel2...) + M1(deel3...) + M2(deel4...)
...
α = α1(deel1...) + α1(deel2...) + α1(deel3...) + α2(deel4...)
...
# ╔═╡ 5a9b1ce5-c92d-4716-aadd-9a468c59b9de
combine(
	groupby(
		select(rvw_volledig, :,
