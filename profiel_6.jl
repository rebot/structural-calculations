...
situatieschets = load("./assets/img/profiel_6.jpg")
...
# ╔═╡ 6452d9f4-c6d3-4b8c-898e-49549deed09b
md"""
!!! warning "Krachtsafdracht"
	De krachtsafdracht van de vloer verloop feitelijk via een driehoekig belastingspatroon. Dit geeft aanleiding tot een maximaal moment Mmax = WxL/6, terwijl we .... ik trek even mijn staart in - TODO
"""

# ╔═╡ b5d75f3f-10e7-49a4-bc9d-ec724d8ce4c9
let
	    content = load("./assets/img/profiel_6_situatieplan.png")
	    HTML("<details><summary><strong>Toon krachtsafdracht</strong></summary>$(html(content))</details>")
end

# ╔═╡ 31851342-e653-45c2-8df6-223593a7f942
md"## Probleemstelling: Eenvoudig opgelegde ligger
Eenvoudig opgelegde ligger met een uniform verdeelde belasting en driehoekige belasting"
...
md"Weergave alle profielen met een breedte dicht bij **14cm**"
...
# ╔═╡ e4b21e41-886b-46ec-9f3a-7812f2d74eae
md"""
De **afdracht** van de bovenliggende muur wordt gesimuleerd door een **verdeelde belasting**, terwijl de afdracht van de **vloer** wordt gesimuleerd door een **driehoekige belasting**, met *piek* in het midden.
"""

# ╔═╡ c4df4b92-a3c6-43bd-a594-9d1f8c76015f
md"In het desbetreffende geval van een driehoekbelasting worden er **twee variabele verdeelde belastingen** naast elkaar gemodelleerd, met een gedeelde maximale waarde. Het aantal parameters herleidt zich tot $L$ en $W$"

# ╔═╡ 7864260e-62ba-4564-acfb-b23bf6afbbc1
@drawsvg begin
	sethue("black");
	endpnts_set1 = (-200, 0), (190, 0);
	pnt_W = Point(0, 0);
	pnts_set1 = Point.(endpnts_set1) |> collect;
	@layer (
		fontsize(20);
		poly(pnts_set1, :stroke);
		circle.(pnts_set1, 4, :fill);
		Luxor.label.(["1", "2"], [:NW, :NE], pnts_set1, offset=15)
	)
	@layer (
		# Verdeelde last 1
		fontsize(16);
		for i = 0:9
			Luxor.arrow(Point(-15 * i, -40/13 * (13 - i)), Point(-15 * i, 0));
		end;
		poly([pnts_set1[1], pnt_W + (0, -40)], :stroke);
		Luxor.label("W", :N, pnt_W + (0, -40), offset=15);
	)
	@layer (
		# Verdeelde last 2
		fontsize(16);
		...
		poly([pnt_W + (0, -40), pnts_set1[2]], :stroke);
	)
	@layer (
		fontsize(16);
		Luxor.translate(0, 10);
		Luxor.arrow(pnts_set1...);
		Luxor.label("L", :SW, pnts_set1[2], offset=15);
	)
end (800) (150)

# ╔═╡ e0ac2fd3-c6ad-42c7-8c44-9dd96336afe3
md"Bijkomend is er de krachtsafdracht van de muur zelf. Deze geeft aanleiding tot een uniform verdeelde last, hiervoor dienen we enkel de parameter $p$ en $L$ te definieëren. Het komt er dus op neer dat alle geometrische parameters te reduceren zijn tot $L$, $W$ en $p$"
...
!!! note "Te definiëren parameters"
	In de tabel met de **randvoorwaarden** (`rvw`) geef je de parameters $L$, $W$ en $p$ in, alsook de grenstoestand (`:UGT` of `:GGT`). De parameters die je moet invullen volgen uit de **generalisering** dat in een volgende paragraaf is opgesteld.
"""
...
geom = (
	L = 3.75, # In afwijking van het situatieplan (daar 1.70m aangeduidt),
	H = 2.5 # Hoogte van muur 11
)
...
opp_kamer2_afdracht = geom[:L] * geom[:L] / 4 # [m²] - Oppervlakte Kamer 2 die afgedragen wordt

# ╔═╡ a4ee26ec-fc28-4ccd-8b09-72241fc1b8c1
opp_kamer3_afdracht = geom[:L] * geom[:L] / 4 # [m²] - Oppervlakte Kamer 3 die afgedragen wordt

# ╔═╡ 65471747-3747-49eb-8e5c-f4847e030974
opp_muur12 = geom[:L] * geom[:H] # [m²] - Oppervlakte van muur 12, muur van 14cm dik

# ╔═╡ 8340c928-46a1-4078-8941-fb286482b732
md"""
!!! info "Assesteen"
	We veronderstellen een gelijkaardig gewicht voor assesteen als een Thermobloc van Ploegsteert. Het gewicht bedraagt 7,6 kg voor een steen met formaat 39x14x19. Per m² komt dit neer op 12.2 stenen (lintvoeg = 1cm; stootvoeg = 1.5cm). De hoeveelheid mortel bedraagt dan ca. 0,01343 m³ aan een volumegewicht van 1600 kg/m³ of rondweg zo'n 25 kg/m². Neem aan weerszijden gips van 1.5cm dikte aan een volumegewicht van 1300 kg/m². In totaal komt een m² Assesteen dus uit op zo'n 157 kg/m². Uit praktische overweging nemen we 175 kg/m² aan. 
!!! info "Potten en balken"
	Het eigengewicht wordt ingeschat op 307 kg/m².
"""
...
combinaties = DataFrame([
	(check=:GGT, naam="p", formule="g_muur"),
	(check=:UGT, naam="p", formule="1.35 * g_muur"),
	(check=:GGT, naam="W", formule="g_vloer + 0.5 * q_vloer"),
	(check=:UGT, naam="W", formule="1.35 * g_vloer + 1.5 * (q_vloer + 0.5 * q_sneeuw)"), 
])
...
!!! tip "Opstellen vergelijkingen"
	Bij het opstellen van de vergelijkingen maak je gebruik van de functies `R11`, `R12`, `V1`, `M1`, `α1` en `v1` voor **gespreide lasten**; van de formules `R21`, `R22`, `V2`, `M2`, `α2` en `v2` voor een **geconcentreerde last** en van de formules `R31`, `R32`, `V3`, `M3`, `α3` en `v3` voor een **varierende belasting**
"""
...
Bereking van **$naam**, een eenvoudig opgelegde ligger **tussen** de **traphal** en de **gemeenschappelijke muur met de buur**. Het profiel ondersteund in theorie enkel de bovenliggende muur, een muur met dikte **14cm** uit assesteen of zindersteen (druksterkte geschat op 2 N/mm²). In de praktijk rekenen we ook een deel van de vloer aan van kamer 2 en 3 mee. 

Van **kamer 2** (4,05 x 3,75m) wordt 23% (±3,51m²) van de last meegenomen die afgedragen wordt op het profiel. Voor **kamer 3** (5,20 x 3,75m) nemen we ook 18% in acht (±3,51m²). De lasten vormen een driehoeksbelasting.

Lasten zijn van permanente aard en nuttige belasting. Geen afdracht van sneeuw komt op dit profiel terrecht. Er wordt gerekened met een **nuttige belasting** van $200 kN/m^2$ en een krachtsafdracht van de vloeroppervlaktes tussen de draagmuren (dus **krachtsafdracht** in **1 richting**), tenzij hier uitdrukkelijk van afgeweken wordt.
"""

# ╔═╡ 7e9d76e1-ee9f-4b3c-bf5f-9b6901f192e6
belastingsgevallen = DataFrame([
	(naam="g_muur", waarde= 1.75 * opp_muur12 / geom[:L], beschrijving="Perm. last - uniform verdeelde belasting - lastendaling"),
	(naam="g_vloer", waarde=3.07 * (opp_kamer2_afdracht + opp_kamer3_afdracht) / geom[:L] * 2, beschrijving="Perm. last - driehoekige last ten gevolge van afdracht - lastendaling"),
	(naam="gp", waarde=profiel[1, "G"] * 0.01, beschrijving="Perm. last - profiel"),
	(naam="q_vloer", waarde=2.00 * (opp_kamer2_afdracht + opp_kamer3_afdracht) / geom[:L] * 2, beschrijving="Var. last - driehoekige last ten gevolge van afdracht - nuttige overlast"),
	(naam="q_sneeuw", waarde=0, beschrijving="Var. last - sneeuwlast")
])
...
belasting = select(maatgevend, 
	:, 
	AsTable([:p, :W]) => 
		ByRow(r -> (r.p * geom[:L] + r.W * geom[:L] / 2) / geom[:L]) => "Gem. Last per m"
)
...
# ╔═╡ b1d52fe3-18e8-47fa-a90b-705c96029324
driehoekig_deel2 = (
	a => geom[:L]/2,
	b => geom[:L],
	p_a => W,
...
R32 = (p_a * (b - a) * (a + b) / 2 + (p_b - p_a) * (b - a) * (a + 2 * b) / 6) / L

# ╔═╡ 90ad790e-78a9-4a65-89ef-887d3ffcc54f
R12 = SymPy.simplify(R32(BC31...))

# ╔═╡ dfd9aed7-9922-4a47-a0b9-20b0bae0ccbf
R2 = R12(uniforme_belasting...) + R32(driehoekig_deel1...) + R32(driehoekig_deel2...)
...
R1 = R11(uniforme_belasting...) + R31(driehoekig_deel1...) + R31(driehoekig_deel2...)
...
V = V1(uniforme_belasting...) + V3(driehoekig_deel1...) + V3(driehoekig_deel2...)
...
M = M1(uniforme_belasting...) + M3(driehoekig_deel1...) + M3(driehoekig_deel2...)
...
α = α1(uniforme_belasting...) + α3(driehoekig_deel1...) + α3(driehoekig_deel2...)
...
BC3 = a => 0, b => 5, L => 5, p_a => _p_a, p_b => _p_b, EI => buigstijfheid
